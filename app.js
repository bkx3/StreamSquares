...
let iconIndex = [];
// Cache fetched Lucide SVG text so searching doesn't re-fetch.
...
function syncControls() {
  ...
  setActiveTab(state.symbol.source === "library" ? "library" : "upload");
}
...
function clearSymbol() {
  ...
  state.symbol.type = null;
  symbolName.textContent = "No symbol loaded";
  render();
  saveState();
}

function clearSymbolAsset() {
  if (symbolUrl) {
...
function handleFile(file) {
  ...
  if (isSvg) {
    ...
    reader.onload = () => {
      ...
      setSymbolFromSvg(svgText, file.name, { normalize: false });
    };
    reader.readAsText(file);
  } else {
    symbolUrl = URL.createObjectURL(file);
    loadSymbolImage(symbolUrl, file.name, "png");
  }
}

function loadSymbolImage(url, name, type = "svg") {
  const img = new Image();
  img.onload = () => {
    symbolImage = img;
...
function setSymbolFromSvg(svgText, name, { normalize }) {
  // Normalize SVGs for Lucide: strip hardcoded colors and use currentColor.
  const normalized = normalize ? normalizeSvg(svgText) : svgText;
  symbolSvgText = normalized;
  const blob = new Blob([normalized], { type: "image/svg+xml" });
  symbolUrl = URL.createObjectURL(blob);
  loadSymbolImage(symbolUrl, name, "svg");
}
...
function setActiveTab(tab) {
  ...
  controlEls.tabUpload.classList.toggle("active", isUpload);
  controlEls.tabLibrary.classList.toggle("active", !isUpload);
  controlEls.tabUpload.setAttribute("aria-selected", isUpload);
  controlEls.tabLibrary.setAttribute("aria-selected", !isUpload);
  controlEls.panelUpload.classList.toggle("active", isUpload);
  controlEls.panelLibrary.classList.toggle("active", !isUpload);
  controlEls.panelUpload.hidden = !isUpload;
  controlEls.panelLibrary.hidden = isUpload;
}

function updateIconGrid(filterText = "") {
  const query = filterText.trim().toLowerCase();
  controlEls.iconGrid.innerHTML = "";
  const filtered = iconIndex.filter((name) => name.toLowerCase().includes(query));
  if (!filtered.length) {
    const empty = document.createElement("div");
    empty.className = "value";
    empty.textContent = "No matches";
    controlEls.iconGrid.appendChild(empty);
    return;
  }
  filtered.forEach((name) => {
    ...
    img.loading = "lazy";
    img.alt = "";
    img.src = `assets/icons/lucide/${name}.svg`;
    button.appendChild(img);
    button.addEventListener("click", () => selectLibraryIcon(name));
    controlEls.iconGrid.appendChild(button);
  });
}

async function loadIconIndex() {
  try {
    const response = await fetch("assets/icons/lucide/index.json");
    if (!response.ok) throw new Error("Icon index missing");
    iconIndex = await response.json();
    updateIconGrid(controlEls.iconSearch.value);
  } catch (error) {
    console.error(error);
    setStatus("Icon library unavailable.", true);
  }
}

async function selectLibraryIcon(name, { resetTransforms = true } = {}) {
  if (!name) return;
  ...
  let svgText = iconCache.get(name);
  if (!svgText) {
    const response = await fetch(`assets/icons/lucide/${name}.svg`);
    if (!response.ok) {
  ...
  clearSymbolAsset();
  setSymbolFromSvg(svgText, name, { normalize: true });
  saveState();
  render();
}

function restoreSymbolFromState() {
  if (state.symbol.source === "library" && state.symbol.libraryName) {
    selectLibraryIcon(state.symbol.libraryName, { resetTransforms: false });
    return;
  }
...
controlEls.tabUpload.addEventListener("click", () => {
  setActiveTab("upload");
});

controlEls.tabLibrary.addEventListener("click", () => {
  setActiveTab("library");
});
